...
```
The regression guard (`benchmark/regression-guard.js`) is order-of-magnitude protection, not a trend tracker: ratio guards compare LazyWatch against the plain-object baselines from the same run (machine speed cancels out; limits ~10x above current ratios), plus conservative absolute ops/sec floors. The read and write benchmarks access an instance created once (ten rounds per iteration) rather than constructing one per iteration — a per-iteration construction made the ratio compare allocation and GC against loop overhead, which varied 7x to 118x across machines for identical code. It prints on every core run but only fails the process under `--check`. When a limit trips because of an intentional trade-off, adjust it in the same commit with an explanation.
...
7. **Utils** (`src/utils.js`) - Helper functions for type checking and cloning
   ...
   - `hasArrayMarker(value)` (numeric `$length` or `$splice` list) tells an array fragment from a plain object; `canMerge(target, source, wholesale)` is the single merge-or-replace rule both appliers use: same-kind containers merge, a real array replaces an object, an unmarked object replaces an array, and a marked fragment replaces a plain object with its revived array
