...
npm run test:size
```
Bundles/minifies via `npx esbuild`, gzips, and fails if the gzipped size exceeds the budget in `scripts/size.js` (9 kB; ~8.1 kB actual as of the JSON-safety work). When the printed size drifts from the "~8 kB min+gzip" claim, update README.md along with the budget.
...
- Array mutations (push, index writes) are tracked via length and index changes. On the wire, every array fragment — index writes, structural ops, and deletions (`#recordDeletion`) alike — carries its length under the reserved `$length` marker (never the `length` data key), so plain data can never be mistaken for a fragment and a pure truncation (`{ $length: n }`) is a self-describing fragment; a fragment's `$length`/`$splice` applied where the slot holds a plain object is dropped (target shape wins)
