...
4. **EventEmitter** (`src/event-emitter.js`) - Batches and emits change notifications
   ...
   - Dispatch is split into production and delivery: `#produce(meta)` consumes the pending diff/inverse as a batch, snapshots the listener list, calls production-time observers (`observe()`, used only by the undo manager so its history is current when a listener calls `undo()`/`group()`), and pushes the batch onto a FIFO `#queue`; `#drain()` delivers queued batches oldest-first under a `#dispatching` flag. A batch produced while a delivery runs (a listener calling `flush`, or `patch`/`overwrite` with metadata) is consumed at the call but delivered only after the current batch reached every listener, by the outermost drain before it returns — delivering on the spot handed later listeners the new batch before the one they were owed (a rejecting listener's inverse overtook the edit it reverted). Outside a listener every emit stays synchronous
   - Listener options: `{ once }` (removed right before its first invocation — never by iterating the live list; nested-path listeners only consume on batches touching their subtree) and `{ signal }` (AbortSignal removal, addEventListener semantics)
   ...
   - Two synchronous emits: `forceEmit(meta)` (the implicit flush behind `silent`, `transaction`, `patch`/`overwrite` with metadata, `createUndoManager`, and the undo manager) bypasses batching, throttle, and debounce but not pause — while paused the split-off batch stays in `#queue` (held, metadata attached, still separate) until `resume()` delivers held batches synchronously or an explicit flush does; `flush(meta)` (`LazyWatch.flush`) also bypasses pause via `#drain(true)`, which forces delivery of everything queued so far (`#forced` counts them, handed to the outer drain when nested). Pause is checked between batches, never within one. Emitting straight through pause, as all of them once did, notified listeners of a paused instance; stashing the pending diff inside the tracker instead is unsound (container-loss registry and inverse completeness are per batch). Batch metadata (an object, by convention `{ origin }`, validated by `LazyWatch.#assertMeta`) is handed to every listener as the third argument; only synchronous emits carry one. `LazyWatch.patch`/`overwrite` with metadata run `#applyTracked`: force-emit pending (untagged), apply, force-emit tagged — so the tagged batch is exactly the applied diff. The UndoManager's injected `flush(meta)` tags undo/redo batches `{ origin: 'undo' | 'redo' }`
...
5. **UndoManager** (`src/undo-manager.js`) - Undo/redo stacks built on inverse diffs
   ...
   - Dependency-injected (subscribe/flush/patch/hasPending/compose/onDispose closures built in the static factory); `subscribe` registers an emitter observer, not a listener, so batches are recorded as they are produced, so the class never touches LazyWatch internals directly; a constructor throw (bad option) restores the instance's `inverseEnabled` in the factory
...
7. **Utils** (`src/utils.js`) - Helper functions for type checking and cloning
   ...
   - `deepClone()` creates copies of objects/arrays for diff storage. Plain data (plain objects, arrays, primitives: all watched state holds) is copied by hand first (`cloneData`, several times faster than `structuredClone` on small values; holes kept, an own `__proto__` key copied as data, a shared sub-object copied twice); anything else, or a nesting past 256 levels (a cycle), takes `structuredClone`, with a manual fallback for plain objects, arrays, Date and RegExp (general-purpose — the latter two are rejected from watched state) and functions by reference. Also backs `LazyWatch.snapshot(watched)`, which returns an independent plain clone of the root or a nested subtree
...
- `splice`/`unshift`/`shift` are intercepted in the `get` trap and recorded as compact `$splice` ops (`[start, deleteCount, items]` triples) instead of per-index writes. The objects stay in their slots and their contents move (slot-merge), which keeps cached child-proxy paths valid — raw splicing would move elements and stale them. On the compact path, which records nothing per slot, `#spliceSlots` does that merge on the raw array directly (`#writeSlot`/`#mergeSlot` reproduce the set trap's and `overwrite`'s wholesale semantics without recording), in the native order so a slot is read before it is written; the fallback paths (inverse on, listeners below, a dirty node) still run the native method through the proxy for per-index recording. `splice` and `shift` return plain copies of what they removed, taken before anything moves (a proxy would address the slot the shift refilled, breaking the usual move); `unshift` returns the length
